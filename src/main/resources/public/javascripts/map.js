...
(function (A, $, L, C, dc) {
  ...
  A.app.instance.addInitializer(function (options) {
    ...
    A.app.map = L.map("map").setView([23.7638, 90.3994], 11);

    L.tileLayer("https://map.barikoi.com/styles/osm-bright/{z}/{x}/{y}.png", {
      attribution:
  ...
  A.app.RoutingSidebar = Marionette.Layout.extend({
    ...
    onShow: function () {
      ...
      $.getJSON("/weeks", function (data) {
        ...
        _this
          .$("#week1FromList")
          .append('<option value="-1">All Weeks</option>');
    ...
    changeFromWeek: function () {
      ...
      if (this.$("#week1FromList").val() > 0) {
        ...
        for (var i in A.app.sidebar.weekList) {
          if (
            A.app.sidebar.weekList[i].weekId >= this.$("#week1FromList").val()
          ) {
            var weekDate = new Date(A.app.sidebar.weekList[i].weekStartTime);
            this.$("#week1ToList").append(
              '<option value="' +
                A.app.sidebar.weekList[i].weekId +
                '">Week of ' +
                (weekDate.getUTCMonth() + 1) +
                "/" +
                weekDate.getUTCDate() +
      ...
      if (this.$("#week2FromList").val() > 0) {
        ...
        for (var i in A.app.sidebar.weekList) {
          if (
            A.app.sidebar.weekList[i].weekId >= this.$("#week2FromList").val()
          ) {
            var weekDate = new Date(A.app.sidebar.weekList[i].weekStartTime);
            this.$("#week2ToList").append(
              '<option value="' +
                A.app.sidebar.weekList[i].weekId +
                '">Week of ' +
                (weekDate.getUTCMonth() + 1) +
    ...
    update: function () {
      ...
      if (!A.app.sidebar.hourlyChart) return;
    ...
    getWeek1List: function () {
      ...
      if (wFrom == -1) {
        ...
        for (var i in A.app.sidebar.weekList) {
          if (
            A.app.sidebar.weekList[i].weekId >= wFrom &&
            A.app.sidebar.weekList[i].weekId <= wTo
          )
            wList.push(A.app.sidebar.weekList[i].weekId);
        }
      }
    ...
    loadChartData: function (data) {
      ...
      if (!this.chartData) {
        ...
        else A.app.sidebar.percentChange = false;
        ...
        this.dailyChart.yAxis().tickFormat(function (d) {
          if (A.app.sidebar.percentChange) return Math.round(d * 100) + "%";
          else return d;
        });
        ...
        if (
          A.app.sidebar.hourExtent &&
          (A.app.sidebar.hourExtent[0] >= 1.0 ||
            A.app.sidebar.hourExtent[1] >= 1.0)
        ) {
          this.hourlyChart.filter(
            dc.filters.RangedFilter(
              A.app.sidebar.hourExtent[0],
              A.app.sidebar.hourExtent[1]
            )
          );
        }
        if (
          A.app.sidebar.dayExtent &&
          (A.app.sidebar.dayExtent[0] >= 1.0 ||
            A.app.sidebar.dayExtent[1] >= 1.0)
        ) {
          this.dailyChart.filter(
            dc.filters.RangedFilter(
              A.app.sidebar.dayExtent[0],
              A.app.sidebar.dayExtent[1]
            )
          );
        }
  ...
  A.app.AnalysisSidebar = Marionette.Layout.extend({
    ...
    update: function () {
      ...
      var url =
        "/weeklyStats?confidenceInterval=" +
        confidenceInterval +
        "&normalizeByTime=" +
        normalizeByTime +
        "&x1=" +
      ...
      if (this.$("#compare").prop("checked")) {
        if (w2List && w2List.length > 0) url += "&w2=" + w2List.join(",");
      }
    ...
    loadChartData: function (data) {
      ...
      if (!this.chartData) {
        ...
        this.dailyChart
          .brush()
          .on("brushend.custom", A.app.sidebar.updateTrafficTiles);
    ...
    addTrafficOverlay: function (hours) {
      if (!A.app.sidebar.filterChanged) return;
      ...
      if (hours && hours.length > 0) hoursStr = hours.join(",");
      ...
      var url =
        "/tile/traffic?z={z}&x={x}&y={y}&confidenceInterval=" +
        confidenceInterval +
        "&normalizeByTime=" +
        normalizeByTime;

      if (hoursStr) url += "&h=" + hoursStr;

      if (w1List && w1List.length > 0) url += "&w1=" + w1List.join(",");
  ...
  A.app.Nav = Marionette.Layout.extend({
    ...
    clickData: function (evt) {
      ...
      A.app.dataOverlay = L.tileLayer("/tile/data?z={z}&x={x}&y={y}").addTo(
        A.app.map
      );
    },
    ...
    onMapClick: function (evt) {
      if (this.startPoint == false) {
        this.startPoint = L.circleMarker(evt.latlng, {
          fillColor: "#0D0",
          color: "#fff",
          fillOpacity: 1.0,
          opacity: 1.0,
        ...
        this.endPoint = L.circleMarker(evt.latlng, {
          fillColor: "#D00",
          color: "#fff",
          fillOpacity: 1.0,
          opacity: 1.0,
    ...
    getRoute: function (hours) {
      ...
      $.getJSON(url, function (data) {
        ...
        A.app.sidebar
          .$("#travelTime")
          .text(Math.round(minutes) + "m " + Math.round(seconds) + "s");
